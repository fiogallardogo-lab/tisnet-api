...
describe('Sprint 5 users, profiles, catalog and legal versions (MySQL e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    for (const [name, isActive] of [
      ...
      const technology = await prisma.technology.create({
        data: { name, isActive, icon: 'test.svg' },
      });
    ...
    try {
      if (prisma) {
        await prisma.user.deleteMany({
          where: { email: { in: [...emails, registrationEmail] } },
        });
      ...
      await app?.close();
      if (previousTerms === undefined) delete process.env.TERMS_VERSION;
      else process.env.TERMS_VERSION = previousTerms;
      if (previousPrivacy === undefined) delete process.env.PRIVACY_VERSION;
      else process.env.PRIVACY_VERSION = previousPrivacy;
    }
  });
  ...
  it.each(roles)(
    ...
    async (role) => {
      ...
      const bodies = {
        CLIENT: { district: 'Lima Sprint 5' },
        DEVELOPER: {
          career: 'Ingeniería',
          technologyIds: [technologyIds[0], technologyIds[0]],
        },
        PRODUCT_OWNER: { bio: 'Producto Sprint 5' },
        ADMIN: { executiveTitle: 'Administración Sprint 5' },
        SUPER_ADMIN: { name: 'Super Admin Sprint 5' },
      };
      const endpoint =
        role === 'SUPER_ADMIN'
          ? ''
      ...
      if (role === 'SUPER_ADMIN') {
        expect(read.body.data.profile).toBeNull();
        expect(read.body.data.user.name).toBe(bodies.SUPER_ADMIN.name);
      } else {
      ...
      const saved = await prisma.user.findUniqueOrThrow({
        ...
        include: {
          clientProfile: true,
          developerProfile: { include: { technologies: true } },
      ...
      if (role === 'SUPER_ADMIN') expect(saved.adminProfile).toBeNull();
    },
  );
  ...
  it('rejects profile role escalation and invalid profile DTOs', async () => {
    await request(app.getHttpServer())
    ...
    await request(app.getHttpServer())
      .patch('/api/v1/users/me/developer-profile')
      .set('Authorization', `Bearer ${tokens.DEVELOPER}`)
      .send({ githubUrl: 'ftp://example.com' })
      .expect(400);
    await patchUser({ userId: userIds.ADMIN, name: 'No permitido' }).expect(
      400,
    );
  });
  ...
  it('registration rejects missing, blank, null, stale or unauthorized legal fields without writes', async () => {
    ...
    for (const field of ['termsVersion', 'privacyVersion'] as const) {
      for (const value of [undefined, '', '  ', null, 'old', 'a'.repeat(51)]) {
        await register({ ...valid, [field]: value }).expect(400);
        expect(
          await prisma.user.findUnique({ where: { email: registrationEmail } }),
        ).toBeNull();
      }
    }
    await register({ ...valid, acceptedTerms: false }).expect(400);
  });
  ...
  it('updates legal acceptance atomically and name-only updates preserve it', async () => {
    ...
    await patchUser({ name: 'Solo nombre S5' }).expect(200);
    ...
    // Legal-only renewal also applies to SUPER_ADMIN, which has no specialized profile.
    await patchUser(legal, 'SUPER_ADMIN').expect(200);
    expect(
      await prisma.user.findUniqueOrThrow({
        where: { id: userIds.SUPER_ADMIN },
      }),
    ).toMatchObject({
      termsVersion: legal.termsVersion,
      privacyVersion: legal.privacyVersion,
  ...
  it('rejects invalid legal updates and preserves every persisted user field', async () => {
    ...
    for (const body of [
      ...
      { ...legal, termsVersion: 'old' },
      { ...legal, privacyVersion: 'old' },
    ]) {
      await patchUser(body).expect(400);
      expect(
        await prisma.user.findUniqueOrThrow({ where: { id: userIds.CLIENT } }),
      ).toEqual(before);
    }
    await patchUser({
