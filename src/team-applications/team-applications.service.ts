...
export class TeamApplicationsService {
  ...
  async findMyInterviews(userId: number) {
    ...
    const applications = await this.prisma.teamApplication.findMany({
      where: {
        assignedAdminProfileId: adminProfile.id,
      },
      select: detailSelect,
      orderBy: { interviewAssignedAt: 'desc' },
    });

    return applications.map((item) => this.toDetail(item));
  }
  ...
  async findMyInterviewDetail(id: number, userId: number) {
    ...
    if (!application) {
      throw new NotFoundException('Postulación no asignada');
    }

    return this.toDetail(application);
  }

  async decideMyInterview(
    id: number,
    ...
    const application = await this.prisma.teamApplication.findFirst({
      ...
      select: { id: true, email: true, requestedRole: true, profile: true },
    });
    if (!application) {
      throw new NotFoundException('Postulación no asignada');
    }
    ...
    const newStatus =
      decision === 'ACCEPTED'
        ? TEAM_APPLICATION_STATUS.ACCEPTED
        : TEAM_APPLICATION_STATUS.REJECTED;

    await this.prisma.teamApplication.update({
      where: { id },
      data: {
        status: newStatus,
        rejectionReason: decision === 'REJECTED' ? reason || 'No cumple con los requisitos técnicos requeridos.' : null,
        rejectedAt: decision === 'REJECTED' ? new Date() : null,
      },
    });

    return this.findMyInterviewDetail(id, userId);
  }
