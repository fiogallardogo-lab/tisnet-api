...
describe('ClientPortalService', () => {
  ...
  it('scopes all private records to the authenticated client and never returns internal user fields', async () => {
    ...
    expect(prisma.project.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: {
          members: {
            some: { userId: 7, memberRole: 'CLIENT', isActive: true },
          },
          status: { not: 'ARCHIVED' },
        },
      }),
    ...
    expect(prisma.quote.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: {
          OR: [
            { prospect: { userId: 7 } },
            { contactEmail: 'lucia@example.test' },
          ],
        },
      }),
    );
    ...
    expect(prisma.publicQuote.findMany).not.toHaveBeenCalled();
  ...
  it('cannot cancel another client’s meeting or a completed meeting', async () => {
    ...
    await expect(service.cancelMeeting(7, 99)).rejects.toBeInstanceOf(
      NotFoundException,
    );
  ...
  it('persists a request under the current account without claiming it is confirmed', async () => {
    ...
    prisma.meeting.create.mockResolvedValue({ id: 22, status: 'PENDING' });
