...
export class ProfilesService {
  ...
  async updateOwnUser(userId: number, dto: UpdateOwnUserDto) {
    ...
    const user = await this.usersService.updateOwnName(userId, dto.name);
    return {
      id: user.id,
      name: user.name,
      email: user.email,
  ...
  async updateProductOwnerProfile(
    ...
    const profile = await this.prisma.productOwnerProfile.upsert({
      where: { userId },
      create: { userId, ...dto },
      update: dto,
    });
  ...
  async updateAdminProfile(userId: number, dto: UpdateAdminProfileDto) {
    const profile = await this.prisma.adminProfile.upsert({
      where: { userId },
      create: { userId, ...dto },
      update: dto,
    });
  ...
  private rethrowProfileConflict(error: unknown): never {
    if (
      ...
      throw new ConflictException('El DNI o RUC ya está registrado');
    }
    throw error;
  }
}
